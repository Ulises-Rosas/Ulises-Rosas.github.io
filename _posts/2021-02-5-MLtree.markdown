...
Printed nodes are from the recursive calls, which is the pre-order traversal of the tree and it is one [topological sorting](https://en.wikipedia.org/wiki/Topological_sorting) of the tree when the queue is updated with left children first. This is useful when implementing non-recursive algorithms, which are more efficient than recursive ones. Now, if we see our tree after the recursive calls:
