...
$$
\begin{align}
\frac{1}{2n}\lVert \mathbf{y} - \mathbf{X}\beta \rVert_2^2 + \lambda \lVert \beta \rVert_1
\end{align}
$$

Its corresponding dual objective function is:
...
Notice that for the constraint, we can apply the definition of the infinity norm as $$\lambda \geq \lvert (\mathbf{\Lambda}^{\top}\mathbf{X})_j  \rvert$$ for *all* $j$. This also implies that $$\lambda \geq \max \lvert (\mathbf{\Lambda}^{\top}\mathbf{X})_j  \rvert$$ for *some* $j$. The right-hand side of the inequality is precisely the definition of the infinity norm (i.e., $\lVert \mathbf{\Lambda}^{\top}\mathbf{X} \rVert_{\infty}$). 
...
$$
\begin{align*}
\mathbf{\Lambda}^t = \frac{1}{n} \mathbf{R}^t
\end{align*}
$$
